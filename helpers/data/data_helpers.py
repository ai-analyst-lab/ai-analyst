...
def list_tables(data_dir=None, files=None):
    """List available table names for a dataset.

    When the dataset's manifest declares a ``files:`` list, that list is the
    source of truth: only those files count as tables. Globbing every CSV in
    the folder is only a fallback for manifests without a ``files:`` list --
    two datasets registered from the same folder must not bleed into each
    other's table inventory.

    Args:
        data_dir: Directory to scan. If None (and ``files`` is None),
            resolves from the active dataset, honoring its manifest's
            ``files:`` list when present.
        files: Explicit list of file paths belonging to the dataset (as in
            the manifest's ``files:`` list). When given, table names come
            from these paths and no directory glob happens.

    Returns:
        Sorted list of table names (filenames without extension).
        Returns an empty list if nothing resolves.
    """
    if data_dir is None and files is None:
        source = detect_active_source()
        data_dir = source.get("csv_path")
        files = source.get("files")
        if data_dir is None and not files:
            return []
...
def detect_active_source():
    ...
    # The manifest's files: list (top-level, connection block, or local_data
    # block) names exactly which files belong to this dataset. Table
    # enumeration must respect it -- see list_tables().
    files = (
        manifest.get("files")
        or conn.get("files")
        or local_data.get("files")
    )
...
def get_connection_for_profiling(source_info=None):
    ...
    # CSV fallback -- honor the manifest's files: list so datasets sharing a
    # folder do not bleed into each other's table inventory.
    csv_dir = source_info.get("csv_path")
    result["type"] = "csv"
    result["csv_dir"] = csv_dir
    result["tables"] = list_tables(csv_dir, files=source_info.get("files"))
    return result
