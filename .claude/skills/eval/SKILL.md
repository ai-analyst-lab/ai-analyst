...
- **Local (default).** Ground truth = `data/eval/ground_truth.yaml`: 10 verified cases over the bundled S&P 500
  data (`data/sp500/`), generated by `data/eval/make_ground_truth.py`. Grading is a relative-tolerance
...
questions = load_questions(split="train")   # [{id, split, question, unit}] from data/eval/ground_truth.yaml
```
...
- Add your own ground-truth cases: copy the shape in `data/eval/ground_truth.yaml` (id, split, question, answer, unit,
