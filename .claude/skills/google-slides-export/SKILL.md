...
Drive files stay under the user's control, with just enough link access for the
Slides API to fetch the image.

1. **Upload each chart PNG to Drive using the chart-to-drive skill**
  (`.claude/skills/chart-to-drive/SKILL.md`). It uploads via the Google MCP tools,
  sets link access, and returns a direct-download URL of the form
  `https://drive.google.com/uc?id=<FILE_ID>` that works in `createImage`.
...
4. **The chart is already in Drive** (step 1), so no extra save step is needed;
  record the Drive file ID in the chart map for reuse.

**Important API constraints:**
- BAD: `data:image/png;base64,...` -> returns 403 Forbidden
- BAD: `http://localhost:...` -> MCP server cannot access local URLs
- OK: a Drive direct-download URL (`https://drive.google.com/uc?id=...`) serving the image
...
Built via `helpers/provenance/provenance_assembler.py`:
```python
from helpers.provenance.provenance_assembler import render_data_stamp
