...
## Purpose
Execute the selected causal inference method using production-grade coded helpers. Does not improvise statistical code — calls functions from `helpers/stats/experiment_stats/causal/`. Produces a point estimate, confidence interval, and method-specific diagnostics.
...
```python
from helpers.stats.experiment_stats.causal import pre_post_analysis
...
```python
from helpers.stats.experiment_stats.causal import did_basic, parallel_trends_test, event_study
...
```python
from helpers.stats.experiment_stats.causal import propensity_match, balance_table, love_plot
...
```python
from helpers.stats.experiment_stats.causal import regression_adjust
...
## Validation
1. **Uses coded helpers** — all statistical computation goes through `helpers/stats/experiment_stats/causal/`. No inline scipy or statsmodels.
