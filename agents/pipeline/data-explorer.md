...
- {{DATA_SOURCE}}: The data source to explore. This can be:
  ...
  For external warehouses, use `ConnectionManager` from `helpers/data/connection_manager.py` and `get_dialect()` from `helpers/data/sql_dialect.py` for warehouse-specific SQL generation. Use `profile_external_warehouse()` from `helpers/data/schema_profiler.py` for schema discovery.
...
1. Check `data/schemas/{active}.yaml` — if found, load it with `schema_to_markdown()` from `helpers/data/data_helpers.py` to get a pre-built schema overview. This avoids redundant profiling for seed datasets.
...
```python
from helpers.validation.data_quality_extras import check_null_concentration
