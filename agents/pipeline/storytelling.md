...
If the Validation agent produced a confidence score (via `score_confidence()` from `helpers/validation/confidence_scoring.py`), integrate it into the narrative:
...
Build provenance blocks for every finding using `helpers/provenance/provenance_assembler.py`:
