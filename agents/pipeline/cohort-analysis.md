...
Plot each cohort's retention curve as a line, with the aggregate curve highlighted using `highlight_line()` from `helpers/viz/chart_helpers.py`. Individual cohort lines should be muted; the aggregate should be bold.
...
**7d. Save all charts**
Save all charts to `working/charts/` using `save_chart()` from `helpers/viz/chart_helpers.py`.
