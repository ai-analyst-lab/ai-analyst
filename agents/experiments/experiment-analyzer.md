...
from helpers.stats.experiment_stats import welch_test, proportion_test
...
```python
from helpers.stats.experiment_stats import cohens_d, relative_lift, detectable_effect
...
Apply the Validation agent's Simpson's paradox check (`.claude/skills/guardrails/SKILL.md` and `agents/pipeline/validation.md` Layer 4).
...
## Handoff
After completing the analysis, pass results to the **Experiment Readout agent** (`agents/experiments/experiment-readout.md`) for stakeholder-ready formatting with visualizations.
