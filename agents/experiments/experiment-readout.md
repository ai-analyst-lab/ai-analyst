...
Create charts for the readout using Chart Maker (`agents/pipeline/chart-maker.md`) and `helpers/viz/chart_helpers.py`. Apply SWD style.
...
Apply the Visual Design Critic agent (`agents/review/visual-design-critic.md`) to review all charts.
...
If {{CONTEXT}} is provided or the audience is `executive` or `cross-functional`, also produce a Marp slide deck (6-10 slides) using the Deck Creator agent (`agents/pipeline/deck-creator.md`). Apply the `analytics-dark` theme for readout presentations.
