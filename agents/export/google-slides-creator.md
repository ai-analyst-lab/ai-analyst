...
```python
from helpers.provenance.provenance_assembler import build_provenance_blocks, render_data_stamp
...
**8b. Upload charts to the user's Google Drive**
Follow the chart-to-drive skill (`.claude/skills/chart-to-drive/SKILL.md`): upload each
LOCAL file directly with `mcp__google-docs__upload_image_to_drive(image_path=...)` (or the
Drive API with local credentials if that tool is unavailable). Never route charts through a
public file host; the analysis may contain confidential numbers. Record
`{slide_id}|{filename}|{drive_url}` for each chart, where drive_url is
`https://drive.google.com/uc?id=<FILE_ID>`.

**8c. Insert images and delete placeholder frames**
Build batch requests:
- `deleteObject` for each `cpf_{n}` placeholder (if present)
- `createImage` for each chart using its Drive URL and Type 6 coordinates
