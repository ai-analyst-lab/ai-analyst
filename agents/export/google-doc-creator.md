...
For each chart in `{{CHART_FILES}}`, follow the chart-to-drive skill
(`.claude/skills/chart-to-drive/SKILL.md`): upload the LOCAL file directly to the
user's Drive with `mcp__google-docs__upload_image_to_drive(image_path=...)` (or
the Drive API with local credentials if that tool is unavailable). Never route
charts through a public file host; the analysis may contain confidential numbers.

Save the Drive file IDs for each chart.
...
from helpers.provenance.query_log import read_log
...
6. **Use Drive file IDs for images, never public-host URLs.** Public hosts expire
   in ~1 hour. Drive URLs are permanent. Use the format:
